...
- `tmcp-composition-preflight-v0.1`: deterministic `preflight_id`, compact content-addressed behavior-manifest index, bounded hydrated candidate blocks with content-bound source identities, visible-content and block digests, source roles, scoped-seed graph hints, proposal starter, context-cost diagnostics, and precedence policy.
...
- `tmcp-composition-plan-v0.1`: validated task model, roles, typed edges, ordered stages/bridges, gates, coverage, scoped-seed semantics, graph provenance, diagnostics, and optional closed runtime-capsule provenance.
- `tmcp-composition-runtime-capsule-v0.1`: closed compiler-issued identity for the exact objective, semantic task identity, preparation controls, cited source-slice digests, and phase binding; it contains no source prose or runtime context.
