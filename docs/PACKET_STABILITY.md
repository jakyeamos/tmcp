...
- `tmcp-composition-preflight-v0.1`: deterministic `preflight_id`, compact content-addressed behavior-manifest index, bounded hydrated candidate blocks with source/block digests, source roles, scoped-seed graph hints, proposal starter, context-cost diagnostics, and precedence policy.
