...
| `/Users/jakyeamos/skills/engineering/refactor-clean/SKILL.md` | Fixture approved for preregistration; packet-probed | One owner, stale-path removal, consumer verification | Pin/archive the future packet attachment, extend the reviewed fixture family set, and create a separately preregistered behavioral plan before any model call. |
...
| --- | --- | --- | --- |
| TMCP packet + `explore-unknowns` Stage-1 bundle | Preregistered, causal launch blocked on baseline receipt | `composition-study-8fd63d3df50f41b0` can test the delivery effect of the byte-pinned source bundle against the same packet. It does not test TMCP live selection, source adherence, or corpus quality. | Run and independently verify the derived 36-cell original-only baseline plan first; then obtain fresh approval for the primary 72 runner and 72 judge calls, followed separately by the 72-trace cost rejudge. |
| TMCP compiler + `refactor-clean` | Fixture approved for preregistration; packet-probed | The packet preserves a consumer-surface verification gate; ownership behavior and the stale-path outcome remain untested. | Pin/archive the future packet attachment, extend the reviewed fixture family set, and require a first-principles bar plus preregistered behavioral plan. |
