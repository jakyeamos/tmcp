...
| --- | --- | --- | --- |
| TMCP packet + `explore-unknowns` Stage-1 bundle | Preregistered, causal launch blocked on baseline receipt | `composition-study-7922312418f1d995` can test the delivery effect of the byte-pinned source bundle against the same packet. It does not test TMCP live selection, source adherence, or corpus quality. | Complete and verify a compatible original-only baseline receipt first; then obtain fresh approval for the primary 72 runner and 72 judge calls, followed separately by the 72-trace cost rejudge. |
