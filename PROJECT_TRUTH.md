...
- Last completed changes: `9bd19d3` fixes a provenance redaction false positive: public one-directory document paths such as `docs/SKILL_EVALUATION_CAMPAIGN_GUIDEBOOK.md` now remain readable in packet citations and instructions, while an opaque basename still redacts. Earlier composition-routing and primary-harness hardening remain local-only and are recorded in the branch history.
