...
- Last completed changes: `b833054` requires source-bundle scoring to rederive every primary preflight's synthetic judge prompt/output-schema contract and valid digest shape before a sidecar can support promotion; `b4a9b77` verifies the primary campaign's isolation, immutable/live-source, completion, and thread-coverage contract; `fe22d47` isolates that primary-contract test coverage; and `581eff3` keeps the source-only campaign-ledger assertion active in the repo while allowing the stripped release package to omit that evidence artifact. The deterministic packet contract and evidence ledger keep the result local-only.
