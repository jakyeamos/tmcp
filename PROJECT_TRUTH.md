...
- Last completed changes: `367914e` archives the exact local primary campaign-harness modules and makes source-bundle promotion verification reject a missing, altered, or inconsistent snapshot; `a0edfb3` separates deterministic campaign planning/readiness from execution while binding it into the primary campaign harness; `439bc07` binds the primary launch and scoring verifier to the campaign policy's runner configurations, judge model/effort, design, and repetitions; and `368896b` embeds the study's immutable input and selected-source binding in the generated plan. The deterministic packet contract and evidence ledger keep the result local-only.
- Verification: 579 repository tests pass (3 skipped); source-bundle regeneration, digest/live-source verification, policy-bound rejudge dry run, verified-score wiring, guidebook/catalog and campaign-ledger boundary checks, API validation, guarded 72-cell dry-run, compiler checks, TMCP doctor, a project-root `reaction` replay, no-write Wizard, branch-fold, and OpenCLI packet replays pass. The reproducible release-package check last passed with 578 packaged tests (9 source-only skips) and will be refreshed. Changed-line Pre-CR passed with an oversized-source advisory for `tmcp_skill_eval_cost_rejudge_source.py`; its primary-harness verification responsibility is being split.
...
- Campaign planning/readiness is a separately bounded module and its file digest is included in each primary manifest's harness binding; the execution protocol remains a compatible re-export surface and is below the source-size quality gate.
...
- Source-bundle scoring now also rechecks the primary launch contract: the exact policy-bound runner configurations, judge model/effort, design, and repetitions; runner/judge schema-preflight roles; the synthetic prompt/output-schema digests and canonical output/event digest shape; prompt-isolation audit; immutable input/live-source report whose input digests and selected sources equal the evaluated plan; zero retained errors; and complete unique-thread coverage before the sidecar may clear promotion gates.
