...
- Last completed changes: `439bc07` binds the primary launch and scoring verifier to the campaign policy's runner configurations, judge model/effort, design, and repetitions; `368896b` embeds the study's immutable input and selected-source binding in the generated plan; `b833054` rederives every primary preflight's synthetic judge prompt/output-schema contract and valid digest shape; and `581eff3` keeps the source-only campaign-ledger assertion active in the repo while allowing the stripped release package to omit that evidence artifact. The deterministic packet contract and evidence ledger keep the result local-only.
- Verification: 577 repository tests pass (3 skipped); source-bundle regeneration, digest/live-source verification, policy-bound rejudge dry run, verified-score wiring, guidebook/catalog and campaign-ledger boundary checks, API validation, guarded 72-cell dry-run, compiler checks, TMCP doctor, a project-root `reaction` replay, no-write Wizard, branch-fold, and OpenCLI packet replays pass. The reproducible release-package check also passes with 577 packaged tests (9 source-only skips). Changed-line Pre-CR passed with its existing `tmcp_skill_eval_campaign_protocol.py` oversized-source warning.
