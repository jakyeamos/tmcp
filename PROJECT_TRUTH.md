...
- Last completed change: `66341f0` preserves a selected branch-fold source's dirty-worktree, remote-truth, and supersession-proof invariants as deterministic packet gates. The deterministic packet contract and evidence ledger keep the result local-only.
- Verification: 573 tests pass (3 skipped); source-bundle regeneration, digest/live-source verification, policy-bound rejudge dry run, verified-score wiring, guidebook/catalog and campaign-ledger boundary checks, API validation, guarded 72-cell dry-run, compiler checks, TMCP doctor, a project-root `reaction` replay, no-write Wizard and branch-fold packet replays, and targeted release-scanner checks pass. The commit hook ran changed-line Pre-CR successfully.
