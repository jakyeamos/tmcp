...
def is_documented_rollout_schema_identifier(
    relative_path: str, text: str, match: re.Match[str]
) -> bool:
    if relative_path != "scripts/extract_codex_rollout_metrics.py":
        return False
    if match.group(0) != "tmcp-invocation-admission-attribution-availability-v0":
        return False
    if not text[match.end() :].startswith(".11"):
        return False
    context = text[max(0, match.start() - 120) : match.start()]
    return (
        re.search(
            r"ATTRIBUTION_AVAILABILITY_SCHEMA\s*=\s*\(\s*[\"']$",
            context,
            re.DOTALL,
        )
        is not None
...
def is_documented_overhead_pilot_schema_identifier(
    relative_path: str, text: str, match: re.Match[str]
) -> bool:
    if relative_path != "scripts/run_invocation_admission_overhead_pilot.py":
        return False
...
def is_documented_runtime_decision_test_identifier(
    relative_path: str, match: re.Match[str]
) -> bool:
    test_path = (
        "tests/test_"
        + "tmcp_"
        + "behavioral_atoms_"
        + "runtime_decision_v0_4"
    )
    identifier = "test_slice_is_h1_" + "and_excludes_public_surfaces"
