...
def summarize(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for variant in ("original", "candidate"):
        ...
        versions[variant] = {
            ...
            "skill_sha256": sorted({cell.get("skill_sha256") for cell in cells}),
        }
...
def main() -> None:
    ...
    if disposition_input.get("schema") != "tmcp-individual-skill-behavior-disposition-input-v0.1":
    ...
    for case_id, registry_case in sorted(registry_cases.items()):
        ...
        if case["observed_skill_failure"] and case["admission_status"] != "case_ready":
            raise ValueError(f"skill failure cannot be claimed for unadmitted case {case_id}")
        cases.append(case)
