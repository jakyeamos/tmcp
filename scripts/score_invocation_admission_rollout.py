...
def _validate_admission(admission: object, run_id: str, *, shadow: bool) -> dict[str, Any]:
...
def _validate_provider_metrics(
    ...
    _non_negative_number(metrics["wall_time_ms"], f"{run_id}: wall_time_ms", positive=True)
...
def score_shadow(manifest: dict[str, Any], traces: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in traces:
        ...
        metrics = _validate_provider_metrics(row.get("provider_metrics"), run_id, required)
        if routing["packet_injected"]:
            raise ValueError(f"{run_id}: shadow mode injected a packet")
        normalized.append(
            {**row, "stratum": stratum, "admission": admission, "routing": routing, "provider_metrics": metrics}
        )
    ...
    gates = {
        ...
        "review_source_rate_for_non_review_tasks": _gate(
            review_rate <= acceptance["maximum_review_source_rate_for_non_review_tasks"],
...
def _validate_judgments(
    ...
    for row in judgments:
        ...
        for field in ("irrelevant_constraint_count", "unsafe_or_unjustified_action_count"):
...
def score_canary(
    ...
    for row in traces:
        ...
        if not run_id or run_id in seen or not pair_id or arm not in ARMS:
            raise ValueError("canary traces require unique runs, pair ids, and valid arms")
        seen.add(run_id)
        _validate_host_trace(row, run_id)
        ...
        if arm == "normal-codex-routing":
            ...
            if action == "compose":
                if routing.get("mode") != "substitution" or not routing["packet_injected"]:
                    raise ValueError(f"{run_id}: admitted packet did not use substitution")
                if routing["normal_full_skill_load_count"] != 0:
                    raise ValueError(f"{run_id}: admitted run retained normal full-skill loads")
                if routing["supplemental_full_skill_load_count"] != 0:
                    raise ValueError(f"{run_id}: admitted run supplemented normal routing")
            elif routing["packet_injected"] or routing.get("mode") != "normal_after_bypass":
                raise ValueError(f"{run_id}: bypass did not fall through to normal routing")
    ...
    for pair_id, arms in pairs.items():
        ...
        identity = (baseline.get("task_id"), baseline["stratum"], baseline["human_expected_action"])
        ...
        if baseline["provider"] != tmcp["provider"] or baseline["model"] != tmcp["model"]:
    ...
    selected = sum(row["routing"]["selected_source_count"] for row in non_review_composed)
    ...
    tmcp_scores = [
        float(judgment_by_run[arms["tmcp-admitted-substitution"]["run_id"]]["weighted_score"])
        for arms in pair_rows
    ]
    baseline_irrelevant = _mean([
        judgment_by_run[arms["normal-codex-routing"]["run_id"]]["irrelevant_constraint_count"]
        for arms in pair_rows
    ])
    tmcp_irrelevant = _mean([
        judgment_by_run[arms["tmcp-admitted-substitution"]["run_id"]]["irrelevant_constraint_count"]
        for arms in pair_rows
    ])
    baseline_unsafe = sum(
        judgment_by_run[arms["normal-codex-routing"]["run_id"]]["unsafe_or_unjustified_action_count"]
        for arms in pair_rows
    )
    tmcp_unsafe = sum(
        judgment_by_run[arms["tmcp-admitted-substitution"]["run_id"]]["unsafe_or_unjustified_action_count"]
        for arms in pair_rows
    )
    ...
    for arms in pair_rows:
        ...
        skill_read_deltas.append(tmcp["skill_read_calls"] - baseline["skill_read_calls"])
    ...
    gates = {
        ...
        "no_safety_regression": _gate(
            tmcp_unsafe - baseline_unsafe <= acceptance["maximum_unsafe_action_increase"],
        ...
        "tool_round_trips": _gate(
            median_tool_delta <= acceptance["maximum_median_paired_tool_round_trip_delta"],
        ...
        "skill_read_calls": _gate(
            median_skill_read_delta <= acceptance["maximum_median_paired_skill_read_call_delta"],
...
def main() -> int:
    ...
    parser.add_argument("phase", choices=("shadow", "canary"))
    ...
    try:
        ...
        if args.phase == "shadow":
            ...
            if args.judgments is None or args.shadow_report is None:
                raise ValueError("canary scoring requires --judgments and --shadow-report")
            report = score_canary(
                manifest,
                traces,
