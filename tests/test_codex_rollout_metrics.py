...
class CodexRolloutMetricsTests(unittest.TestCase):
    ...
    def test_builds_redacted_unavailable_attribution_receipt(self) -> None:
        extracted = extractor.extract_turn(self._write_rollout(), "turn-1")

        receipt = extractor.build_attribution_availability_receipt(extracted, None)

        self.assertEqual(
            receipt,
            {
                "schema": extractor.ATTRIBUTION_AVAILABILITY_SCHEMA,
                "session_id": "session-1",
                "turn_id": "turn-1",
                "attribution_status": "unavailable",
                "scorer_ready": False,
                "disposition": "unavailable-attribution",
                "missing_host_metrics": list(extractor.HOST_COUNTER_FIELDS),
                "promotion_authorized": False,
                "canary_authorized": False,
            },
        )
        rendered = json.dumps(receipt)
        self.assertNotIn("provider_metrics", rendered)
        self.assertNotIn("rollout_tool_calls", rendered)
        self.assertNotIn("openai", rendered)
        self.assertNotIn("private", rendered)

    def test_builds_complete_zero_and_exact_attribution_receipts(self) -> None:
        for calls, tokens, expected in (
            (0, 0, "complete-zero"),
            (1, 25, "complete-exact"),
        ):
            with self.subTest(expected=expected):
                extracted = extractor.extract_turn(self._write_rollout(), "turn-1")
                finalized = extractor.finalize_terminal_turn(
                    extracted,
                    self._host_observation(
                        skill_read_calls=calls,
                        skill_read_input_tokens=tokens,
                    ),
                )

                receipt = extractor.build_attribution_availability_receipt(
                    extracted, finalized
                )

                self.assertEqual(receipt["attribution_status"], expected)
                self.assertTrue(receipt["scorer_ready"])
                self.assertFalse(receipt["promotion_authorized"])
                self.assertFalse(receipt["canary_authorized"])
                self.assertNotIn("provider_metrics", receipt)

    def test_cli_writes_unavailable_receipt_then_fails_closed(self) -> None:
        rollout = self._write_rollout()
        with tempfile.TemporaryDirectory() as directory:
            receipt_path = Path(directory) / "availability.json"
            with patch.object(
                sys,
                "argv",
                [
            ...
            self.assertEqual(receipt["attribution_status"], "unavailable")
            self.assertFalse(receipt["scorer_ready"])

    def test_cli_never_overwrites_or_collides_availability_receipt(self) -> None:
        rollout = self._write_rollout()
        with tempfile.TemporaryDirectory() as directory:
            receipt_path = Path(directory) / "availability.json"
