"""Versioned H1, H2, and private opt-in H3 typed atom registry records."""
...
def _atom(
    *,
    ...
    input_policy: str = "accept only scoped, owner-recorded evidence; never infer from harvested wording",
    renderable_to: Iterable[str] = ("packet", "receipt", "advisory_trace"),
) -> TypedAtom:
    return TypedAtom(
        ...
        rendering_boundary=RenderingBoundary(
            renderable_to=_unique(renderable_to),
            forbidden=(
                "runtime_instruction",
                "provider_prompt",
                "provider_outcome",
...
@lru_cache(maxsize=1)
def build_h3_registry() -> AtomRegistry:
    """Extend H2 with the two H3 atoms for private static evaluation only."""

    secret_boundary = _atom(
        stable_identity=SECURITY_SECRET_BOUNDARY_ID,
        family="security_privacy",
        target="authorized secret, auth, payment, and private-data handling boundaries",
        risk=(
            "unauthorized access",
        ...
        expected_output=(
            "authorized boundary map",
            "observed-versus-inferred risk separation",
            "ordered remediation",
        ),
        positive=(
            "an authorized security or privacy audit names an auth, secret, payment, or private-data boundary",
        ),
        negative=(
            "no access, secret, privacy, or sensitive-data boundary is in scope",
        ),
        ambiguous=(
            "credential-like or private-data signals exist without an authorized review scope",
        ),
        ambiguous_action="stop and request an authorized scope and owner",
        dependencies=(PROCESS_READ_ID, SECURITY_REDACTION_ID),
        conflicts=(
            ConflictSpec(
                stable_identity="conflict.security_privacy.inferred_authority",
                when="authorization is inferred rather than supported by owned evidence",
                resolution="stop/request explicit scope or omit sensitive evidence",
            ),
        ),
        required_inputs=(
            RequiredInput("authorized_scope", "owner-bound review scope"),
            RequiredInput("boundary_evidence", "auth or data-flow evidence"),
        ),
        required_reads=(
            "skills/tmcp-security-privacy-audit/SKILL.md",
        ...
        stop_conditions=(
            "authorized scope is absent or contradictory",
        ...
        token_max=50,
        allowed_phases=_ALLOWED_PHASES,
        supersedes=("domain.security_privacy.secret_boundary@0.3.0",),
        input_policy="treat sensitive input as untrusted until scope, owner, and redaction are verified",
        renderable_to=("advisory_trace",),
    )
    evidence_ladder = _atom(
        stable_identity=RELEASE_EVIDENCE_LADDER_ID,
        family="release_readiness",
        target="quality ladder, launch blockers, and handoff evidence completeness",
        risk=(
            "partial evidence represented as complete",
            "stale verification",
            "undisposed required gate",
        ),
        expected_output=(
            "passed, failed, blocked, not-run, and unknown gate inventory",
            "ordered remediation slices",
            "acceptance checks",
        ),
        positive=(
            "release gaps, launch blockers, quality-ladder coverage, or handoff acceptance are in scope",
        ),
        negative=("no release, handoff, or launch acceptance decision is in scope",),
        ambiguous=("partial checks exist but their scope or freshness is unknown",),
        ambiguous_action="stop and identify missing evidence and freshness",
        dependencies=(PROCESS_READ_ID, RELEASE_SHIP_GATE_ID),
        conflicts=(
            ConflictSpec(
                stable_identity="conflict.release_readiness.partial_evidence",
                when="a partial check is represented as a complete quality ladder",
                resolution="split evidence into passed, missing, blocked, and unknown gates",
            ),
        ),
        required_inputs=(
            RequiredInput("quality_ladder", "required check inventory"),
            RequiredInput(
                "verification_freshness", "command output or timestamped evidence"
            ),
        ),
        required_reads=(
            "skills/tmcp-release-readiness/SKILL.md",
        ...
        stop_conditions=(
            "freshness or scope of the quality ladder is unknown",
        ...
        token_max=52,
        allowed_phases=_ALLOWED_PHASES,
        supersedes=("domain.release_readiness.evidence_ladder@0.3.0",),
        input_policy="quality output is evidence only when command, scope, and result are captured",
        renderable_to=("advisory_trace",),
    )
    return AtomRegistry((*build_h2_registry().atoms, secret_boundary, evidence_ladder))
